<?php
	...
	class userController extends mainModel{
		...
		public function registrarUsuarioControlador(){
		    ...
		    if($this->verificarDatos("[a-zA-Z0-9]{5,20}",$usuario)){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
		    ...
		    if($this->verificarDatos("[a-zA-Z0-9.-]{8,15}",$clave1) || $this->verificarDatos("[a-zA-Z0-9.-]{8,15}",$clave2)){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
		...
		# Verificando email #
		if ($email != "") {
    	// Validar estructura básica de email
    	if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        // Validar que contenga "@" y termine en ".com"
        if (preg_match('/^[^@]+@[^@]+\.(com)$/', $email)) {
            ...
            // Error si no termina en .com
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
                "texto" => "El correo electrónico debe contener '@' y terminar en '.com'",
                "icono" => "error"
            ];
            return json_encode($alerta);
            exit();
				...
				foreach($datos as $rows){
					$tabla.='
						<tr class="has-text-centered" >
							...
							<td>
			                    <a href="'.APP_URL.'index.php?views=userPhoto/'.$rows['usuario_id'].'/" class="button is-info is-rounded is-small">
			                    	<i class="fas fa-camera fa-fw"></i>
			                    </a>
			                </td>
			                <td>
			                    <a href="'.APP_URL.'index.php?views=userUpdate/'.$rows['usuario_id'].'/" class="button is-success is-rounded is-small">
